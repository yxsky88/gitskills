...
var province_id,city_id,district_id,province_name,city_name,district_name,twon_name;
locationInitialize();
//ajaxProvince(); //旧的使用
ajaxDispatching();
getProvince();
...
function ajaxDispatching() {
  if(typeof (auto_bd) != 'undefined'  && auto_bd>0 ) return; // 百度定位中，没出定位结果返回
  ...
  if (typeof(goods_id) != 'undefined' && (region_id != '' || auto_bd == 3)) {
    $.ajax({
      type: "POST",
      dataType: 'json',
      ...
      success: function (data) {
        ...
        twon_name = twon_name?twon_name:'';
        $('#address').text(province_name + city_name + district_name + twon_name);
        if(goods_prom_type == 4){
          $('#dispatching_desc').hide();
        }
        if(auto_bd == 3){
          district_id = data.addr_id.district_id;
          setCookies('district_id', district_id, 30 * 24 * 60 * 60 * 1000);
        }else{
          // 下次打开时，是使用上次选择的地址,还是每次重新定位？每次定位，则删除district_name
          setCookies('province_name', province_name, 30 * 24 * 60 * 60 * 1000);
          setCookies('city_name', city_name, 30 * 24 * 60 * 60 * 1000);
          setCookies('district_name', district_name, 30 * 24 * 60 * 60 * 1000);
          setCookies('district_id', district_id, 30 * 24 * 60 * 60 * 1000);
          //delCookie('district_name'); // 打开则每次定位
        }
      }
    });
  }
...
{
  if(typeof (auto_bd) != 'undefined'  && auto_bd>0 ) return;// 如果定位中，则返回
  province_id = getCookie('province_id');
  city_id = getCookie('city_id');
  district_id = getCookie('district_id');
  province_name = getCookie('province_name');
  city_name = getCookie('city_name');
  district_name = getCookie('district_name');
  //twon_name = getCookieByName('twon_name');
  if(province_name==null || city_name==null || district_name==null){
    province_id = 1;
    city_id = 2;
    district_id = 3;
    province_name = '北京市';
    ...
    $.ajax({
        ...
        success: function(data){
            ...
            if(data.status == 1){
                ...
                for(var i = 0;i < province_count;i++){
                    // province_html += '<p data-id="'+data.result[i].id+'">'+data.result[i].name+'</p>';
                    province_html += '<div class="sel_info" data-id="'+data.result[i].id+'" ><div class="sel_pri_name"> '+data.result[i].name+'</div><div ><i class="Mright Mrightt" style="float:right" ></i></div></div>';
                }
            }
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      if (data.status == 1) {
        var city_count = data.result.length;
        for (var i = 0; i < city_count; i++) {
          city_html += '<div class="sel_info" data-id="'+data.result[i].id+'" ><div class="sel_city_name"> '+data.result[i].name+'</div><div ><i class="Mright Mrightt" style="float:right" ></i></div></div>';
        }
      }
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.status == 1) {
                var district_count = data.result.length;
                for (var i = 0; i < district_count; i++) {
                    district_html += '<div class="sel_info" data-id="'+data.result[i].id+'" ><div class="sel_district_name"> '+data.result[i].name+'</div><div ><i class="Mright Mrightt" style="float:right" ></i></div></div>';
                }
            }
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.status == 1) {
                ...
                if(twon_count < 1){
                	$('.add_show .show_text').eq(3).empty();
                    $('.add_show .show_text').eq(2).removeClass("no_sel").addClass("sel_show");
                	twon_id = 0;
                    twon_name = '';
                    ajaxDispatching();
                	...
                	for (var i = 0; i < twon_count; i++) {
                        twon_html += '<div class="sel_info" data-id="'+data.result[i].id+'" ><div class="sel_twon_name"> '+data.result[i].name+'</div><div ><i class="Mright Mrightt" style="float:right" ></i></div></div>';
                    }
